...
def user_registration(username: str, password: str) -> dict[str: str, str: int] | None:
    return simplification(post(template(f"registration/{username}/{password}")))


def user_log(side: str, username: str, password: str) -> int | dict[str: str, str: int] | None:
    return simplification(post(template(f"log/{side}/{username}/{password}")))


def user_delete(username: str, villain_username: str) -> dict[str: str, str: int] | None:
    return simplification(delete(template(f"delete_user/{username}/{villain_username}")))


def room_all(is_open: bool, names: bool) -> dict[str: list[int, list[str]]] | dict[str: str, str: int]:
    return get(template(f"get_rooms/{str(is_open)[0].lower()}/{str(names)[0].lower()}"))


def room_create(user_id: int, user_limit: int) -> int | dict[str: str, str: int]:
    return post(template(f"create_room/{user_id}/{user_limit}"))


def room_search(room_id: int) -> bool | dict[str: str, str: int]:
    return get(template(f"search_room/{room_id}"))


def room_user_ids(room_id: int) -> list[str] | dict[str: str, str: int]:
    return get(template(f"room_users/{room_id}"))


def room_delete(room_id: int) -> int | dict[str: str, str: int]:
    return simplification(delete(template(f"delete_room/{room_id}")))


def room_enter(room_id: int, user_id: int) -> dict[str: str, str: int] | None:
    return simplification(put(template(f"enter_room/{room_id}/{user_id}")))


def room_enter_field(room_id: int, user_id: int) -> dict[str: str, str: int] | None:
    return simplification(put(template(f"field_enter/{room_id}/{user_id}")))


def room_leave(room_id: int, user_id: int) -> dict[str: str, str: int] | None:
    return simplification(put(template(f"leave_room/{room_id}/{user_id}")))


def room_can_move(room_id: int) -> bool | dict[str: str, str: int]:
    return get(template(f"can_move/{room_id}"))


def room_move(room_id: int, user_id: int, platform_position_y: int) -> dict[str: str, str: int] | None:
    return simplification(put(template(f"move/{room_id}/{user_id}/{platform_position_y}")))
