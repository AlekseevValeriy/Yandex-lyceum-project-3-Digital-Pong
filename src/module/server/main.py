...
# room
@app.route('/get_rooms/<string:open>/<string:names>', methods=["GET"])
@catch_error
def get_rooms(open: str = " ", names: str = " ") -> Response:
...
@app.route('/leave_room/<int:room_id>/<int:user_id>', methods=["PUT"])
@catch_error
def leave_room(room_id: int, user_id: int, message: int) -> Response:
    room = db_session_app.query(Room).filter(Room.id == room_id).first()
    if not room: raise RoomNotExists
